...
\begin{chineseabstract}
    在科技不断发展的今天，人们的日常生活每天都会产生海量数据，而对这些数据资源的分析应用已经创造了大量的经济价值、社会价值。对于中心化的数据资源，可以使用现有的大数据分析技术对其进行价值挖掘。而地理上分布于不同组织及个人的海量数据却难以实现高效联合应用。因为受限于成本，隐私、规范等诸多社会因素，数据拥有者们可能并不愿意将数据直接共享。这造成了“数据孤岛”的现象。为此，本文描述了一种潜在的新数据价值共享计算模式，并针对两种不同的具体计算场景设计了性质良好的拍卖激励机制。本文的具体工作如下：
    \begin{enumerate}
        \item 描述了多方数据价值共享计算模式。针对简单可并行的计算任务，首先对问题进行形式化建模，然后分析了数据量约束、多重指标约束、机构约束三种具体应用实例，并设计了计算有效、真实、社会福利最大化的DSIC拍卖激励机制。模拟实验说明了机制的有效性及良好性能。
        \item 针对依赖相关的计算任务，本文首先分析了以时间限制作为重要指标，以AOE网对任务流程进行建模。论证了相应社会福利最大化问题的NP困难性。然后对一类贪心思路进行证否，并给出了基于贪心思想的启发式算法来近似求解社会福利最大化问题。最后，给出了考虑任务方收益的最优拍卖机制。模拟实验说明了两种机制的有效性。
        \item 本文最后设计并实现了数据价值共享拍卖系统，将上述计算场景进行具体的应用。对激励拍卖系统的重要流程进行界面展示。
    \end{enumerate}
...
With the continuous development of science and technology, people's daily life produces massive data every day, and the analysis and application of these data resources have created a large number of economic and social values.  For centralized data resources, the existing big data analysis technology can be applied for value mining.  However, the massive data geographically distributed in different organizations and individuals are difficult to realize efficient joint application.  Due to many social factors such as cost, privacy, and regulation, data owners may not be willing to share data directly.  This resulted in the phenomenon of "data island".  Therefore, this thesis describes a potential new data value sharing scheme, and designs an ideal auction incentive mechanism for two different specific  scenarios.  The specific work of this thesis is as follows:
\begin{enumerate}
\item Firstly, it describes the multi-party data value sharing scheme.  For simple and parallelizable computation tasks, the formal modeling of the problem is carried out, then three specific application instances of data volume constraint, multiple indicators constraint and organization constraint are analyzed, and a DSIC  incentive auction mechanism with effective computation, truthfullness as well as maximum social welfare is designed.  Simulation experiments show the effectiveness and good performance of the mechanism.
\item For dependency-related computing tasks, this paper first analyzes the modeling of task flow using AOE network with time constraints as an important indicator.  The NP difficulty of the corresponding social welfare maximization problem is demonstrated.  Then, we prove the ineffectiveness of some greedy thoughts and give another greedy heuristic algorithm which works well to approximately solve the social welfare maximization problem.  Finally, the optimal auction mechanism considering the revenue of the task initiator is given.  Simulation experiments show the effectiveness of the two mechanisms.
\item At last, this paper designs and implements a data value sharing auction system, and applies the above scenarios to specific applications.  The crucial process of the auction is partially displayed .
\end{enumerate}
...
\begin{figure}[h]
    \includegraphics[with=400pt]{exp/exetime.pdf}
