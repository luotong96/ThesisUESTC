...
\begin{algorithm}[H]
    ...
    init $lst =\{(bid_i-a,timefromagent_i-b)\}$\;

    置所有子任务为未访问状态。

    \While{$lst$不为空}
    {
        根据第一关键字选出$lst$中的最大值对应的参与者$k$。\;
        然后在$lst$中删除参与者$k$\;
...
类似的，时间复杂度为$O(|E|*|V|+n)$，n为参与者数量。接下来说明分配规则的单调性。
